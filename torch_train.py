...
def configure_training():
    ...
    optimizer = tf.keras.optimizers.SGD(
        learning_rate=config.LEARNING_RATE, momentum=0.9, weight_decay=1e-4
    )
...
def get_callbacks():
    ...
    callbacks = [
        ...
        TensorBoard(
            log_dir=config.PROJECT_ROOT_DIR + "/results/tensorboard", histogram_freq=1
        ),
        CSVLogger(
            config.PROJECT_ROOT_DIR + "/results/tf_training_log.csv"
        ),  # Simple CSV backup
        TrainingLogger(
            config.PROJECT_ROOT_DIR + "/results/tf_history.json"
        ),  # Custom JSON logger
    ]
    return callbacks
